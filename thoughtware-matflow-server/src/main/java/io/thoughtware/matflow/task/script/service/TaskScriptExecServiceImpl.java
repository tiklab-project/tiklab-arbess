...
public class TaskScriptExecServiceImpl implements TaskScriptExecService {
    ...
    public boolean scripts(String pipelineId, Tasks task , String taskType) {
        ...
        if (type.equals(PipelineFinal.TASK_SCRIPT_BAT)){
            name = "Bat脚本";
        }
        if (type.equals(PipelineFinal.TASK_SCRIPT_SHELL)){
            name = "Shell脚本";
        }
        ...
        if (systemType == 1 && (type.equals(PipelineFinal.TASK_SCRIPT_SHELL)) ){
            tasksInstanceService.writeExecLog(taskId, PipelineUtil.date(4)+ "Windows系统无法执行Shell脚本。");
            return false;
        }
        if (systemType == 2 &&  ( type.equals(PipelineFinal.TASK_SCRIPT_BAT) )){
