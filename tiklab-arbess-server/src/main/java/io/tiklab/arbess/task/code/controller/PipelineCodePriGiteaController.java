...
import io.tiklab.arbess.task.code.service.TaskCodePriGiteaService;
...
/**
 * @pi.protocol: http
 * @pi.groupName: 流水线集成gitea控制器
 */
@RestController
@RequestMapping("/code/third/pri/gitea")
public class PipelineCodePriGiteaController {

    @Autowired
    TaskCodePriGiteaService priGiteaService;

    /**
     * @pi.name:获取自建Gitea所有仓库
     * @pi.url:/code/third/gitea/findStoreHouseList
     * @pi.methodType:post
    ...
    public Result<List<ThirdHouse>> findStoreHouseList(@RequestBody @Valid @NotNull ThirdQuery thirdQuery){
        ...
        List<ThirdHouse> allRepository = priGiteaService.findStoreHouseList(thirdQuery);
    ...
    /**
     * @pi.name:获取仓库分支信息
     * @pi.url:/code/third/gitea/findHouseBranchList
     * @pi.methodType:post
     * @pi.request-type: formdata
     * @pi.param: name=authId;dataType=string;value=authId;
    ...
    public Result<List<ThirdBranch>> findHouseBranchList(@RequestBody @Valid @NotNull ThirdQuery thirdQuery){
        ...
        List<ThirdBranch> allRepository = priGiteaService.findStoreBranchList(thirdQuery);
